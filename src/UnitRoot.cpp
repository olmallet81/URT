...
template <typename T>
inline UnitRoot<T>::UnitRoot(const Vector<T>& data, int lags, const std::string& trend, bool regression)
{
   // controlling data is a Vector and not a Matrix
   // Armadillo will convert the Vector into a Matrix, we do not want that
   // Blaze will detect if a Matrix is passed instead of a Vector
   // Eigen will take the first Matrix column to construct a Vector
   #ifdef USE_ARMA
   if (data.n_cols > 1) {
      throw std::invalid_argument("\n  ERROR: in UnitRoot::UnitRoot(), data serie in Unit-Root test cannot be a matrix, please provide a vector as first argument.\n\n");
   }
   #endif
   this->data = data;       
   set_data();
...
template <typename T>
inline UnitRoot<T>::UnitRoot(const Vector<T>& data, const std::string& method, const std::string& trend, bool regression)
{
   // controlling data is a Vector and not a Matrix
   // Armadillo will convert the Vector into a Matrix, we do not want that
   // Blaze will detect if a Matrix is passed instead of a Vector
...
// get test statistic
template <typename T>
inline const T& UnitRoot<T>::get_stat() const
{
   return stat;
}
...
// get test pvalue
template <typename T>
inline const T& UnitRoot<T>::get_pval() const
{
   return pval;
}
...
template <typename T>
inline const OLS<T>& UnitRoot<T>::get_ols() const
{
   return *result;
}
...
template <typename T>
inline const std::vector<std::string>& UnitRoot<T>::get_trends() const
{
   return valid_trends;
}
...
template <typename T>
inline void UnitRoot<T>::set_data()
{
    ptr = &data;
}
...
template <typename T>
inline void UnitRoot<T>::set_lags()
{
   if (!optim) {
...
template <typename T>
inline void UnitRoot<T>::set_lags_type()
{
...
template <typename T>
inline void UnitRoot<T>::set_level()
{
...
template <typename T>
inline void UnitRoot<T>::set_method()
{
...
template <typename T>
inline void UnitRoot<T>::set_niter()
{
...
template <typename T>
inline void UnitRoot<T>::set_test_type()
{
...
template <typename T>
inline void UnitRoot<T>::set_trend()
{
...
template <typename T>
inline void UnitRoot<T>::ols_detrend()
{  
   // initializing w 
   #ifdef USE_ARMA
...
template <typename T>
inline void UnitRoot<T>::gls_detrend()
{
   T alpha;
   int nc;
   // detrend case
...
template <typename T>
inline void UnitRoot<T>::adf_regression()
{
   // number of lines
   int nr = nobs - lags - 1;
   // number of columns
...
template <typename T>
inline void UnitRoot<T>::initialize_adf()
{
...
template <typename T>
inline void UnitRoot<T>::optimize_lag()
{
...
template <typename T>
inline T UnitRoot<T>::IC(const std::shared_ptr<OLS<T>>& res)
{
...
template <typename T>
inline T UnitRoot<T>::MIC(const std::shared_ptr<OLS<T>>& res)
{ 
   #ifdef USE_ARMA
   // getting number of residuals
   int n = res->resid.n_elem;
...
template <typename T>
inline void UnitRoot<T>::select_lag()
{
...
// compute ADF tests 
template <typename T>
inline void UnitRoot<T>::compute_adf()
{
...
template <typename T>
inline void UnitRoot<T>::run_bootstrap()
{  
...
template <typename T>
inline void UnitRoot<T>::compute_cv()
{
...
template <typename T>
inline void UnitRoot<T>::compute_pval()
{
...
// compute p-value
template <typename T>
inline const T& UnitRoot<T>::pvalue()
{
   if (!bootstrap) {
