...
template <class T>
inline OLS<T>::OLS(const Vector<T>& y, const Matrix<T>& x, bool stats)
{ 
   // controlling x and y dimensions
   #ifdef USE_ARMA
   if (y.n_cols > 1) {
      throw std::invalid_argument("\n  Error: in OLS::OLS(), dependent variable in OLS regression cannot be a matrix, please provide a vector as first argument.\n\n");
...
template <typename T>
inline void OLS<T>::get_stats(const Vector<T>& y, const Matrix<T>& x)
{
   // number of variables
   this->nvar = nreg;
